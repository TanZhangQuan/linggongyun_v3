...
public class LianLianPayServiceImpl extends ServiceImpl<LianlianpayDao, Lianlianpay> implements LianLianPayService {
    ...
    public ReturnJson addLianlianPay(String merchantId, AddLianLianPay addLianLianPay) {
        ...
        lianlianpay = this.getOne(new QueryWrapper<Lianlianpay>()
                .lambda().eq(Lianlianpay::getCompanyId, merchant.getCompanyId()));
        if (lianlianpay == null) {
    ...
    public ReturnJson merchantPay(String merchantId, String payPassWord, String paymentOrderId) throws CommonException {
        ...
        Lianlianpay lianlianpay = this.getOne(new QueryWrapper<Lianlianpay>()
                .lambda().eq(Lianlianpay::getCompanyId, paymentOrder.getCompanyId()));
        if (lianlianpay == null) {
            log.error("连连账号不存在！");
        ...
        if (StringUtils.isEmpty(res)) {
            Map<String, String> queryPaymentResult = this.queryPayment(lianlianpay.getOidPartner(),
                    no_order, lianlianpay.getPrivateKey());
            if ("0000".equals(queryPaymentResult.get("ret_code"))) {
                if ("SUCCESS".equals(queryPaymentResult.get("result_pay").toUpperCase())
                        || "PROCESSING".equals(queryPaymentResult.get("result_pay").toUpperCase())) {
                    paymentOrder.setPaymentOrderStatus(4);
                    paymentOrderDao.updateById(paymentOrder);
        ...
        if ("0000".equals(result.get("ret_code"))) {
            ...
            Map<String, String> queryPaymentResult = this.queryPayment(lianlianpay.getOidPartner(), no_order,
                    lianlianpay.getPrivateKey());
            if ("0000".equals(queryPaymentResult.get("ret_code"))) {
                if ("SUCCESS".equals(queryPaymentResult.get("result_pay").toUpperCase())
                        || "PROCESSING".equals(queryPaymentResult.get("result_pay").toUpperCase())) {
                    paymentOrder.setPaymentOrderStatus(4);
    ...
    public void merchantNotifyUrl(HttpServletRequest request) {
        ...
        try {
            ...
            if (!signCheck) {
                ...
                paymentOrder.setPaymentOrderStatus(1);
                paymentOrderDao.updateById(paymentOrder);
                senMsg("订单" + paymentOrder.getId() + "数据异常，请联系服务人员！", result.get("no_order"),
                        "0", UserType.ADMIN, paymentOrder.getMerchantId(), UserType.MERCHANT);
                return;
            }
            if (paymentOrder == null) {
                log.error("支付订单为空！");
...
//            paymentHistoryService.saveOrUpdate(paymentHistory);
            ...
            if ("SUCCESS".equals(result.get("result_pay").toUpperCase())) {
                ...
                paymentOrderDao.updateById(paymentOrder);
                senMsg("订单" + paymentOrder.getId() + "支付成功！", result.get("no_order"),
                        "0", UserType.ADMIN, paymentOrder.getMerchantId(), UserType.MERCHANT);
                LianlianpayTax lianlianpayTax = lianlianpayTaxDao.selectOne(new QueryWrapper<LianlianpayTax>()
                        .lambda().eq(LianlianpayTax::getTaxId, paymentOrder.getTaxId()));
                List<PaymentInventory> paymentInventories = paymentInventoryDao.selectList(new QueryWrapper<PaymentInventory>()
                        .lambda().eq(PaymentInventory::getPaymentOrderId, panymentOrderId));
                //给创客付款
                ...
                for (PaymentInventory paymentInventory : paymentInventories) {
                    synchronized (paymentInventory.getId().intern()) {
                        ...
                        if (worker == null) {
                            log.error("创客不存在" + "\t总包ID:" + panymentOrderId + "\t支付清单ID:" + paymentInventory.getId()
                                    + "\t创客姓名:" + worker.getAccountName());
                            paymentInventory.setPaymentStatus(-1);
                            paymentInventoryDao.updateById(paymentInventory);
                            CommonMessage commonMessage = getCommonMessage("ID为" + paymentInventory.getWorkerId()
                                            + "，姓名为" + paymentInventory.getWorkerName() + "创客不存在，请先导人创客",
                                    "", "0", UserType.ADMIN, paymentOrder.getMerchantId(), UserType.MERCHANT);
                            commonMessageList.add(commonMessage);
                            continue;
                        }
                        if (!(worker.getAgreementSign() == 2 && worker.getAttestation() == 1)) {
                            log.error("创客未认证" + "\t总包ID:" + panymentOrderId + "\t支付清单ID:" + paymentInventory.getId()
                                    + "\t创客姓名:" + worker.getAccountName());
                            paymentInventory.setPaymentStatus(-1);
                            paymentInventoryDao.updateById(paymentInventory);
                            CommonMessage merchantCommonMessage = getCommonMessage("ID为" + paymentInventory.getWorkerId()
                                            + "，姓名为" + paymentInventory.getWorkerName() + "的创客未认证，只有完成实名认证和签署加盟合同后才可以进行发薪",
                                    "", "0", UserType.ADMIN, paymentOrder.getMerchantId(), UserType.MERCHANT);
                            CommonMessage workerCommonMessage = getCommonMessage("您现在未认证，请完成实名认证和签署加盟合同后，才可以进行发薪操作",
                                    "", "0", UserType.ADMIN, worker.getId(), UserType.WORKER);
                            commonMessageList.add(merchantCommonMessage);
                            commonMessageList.add(workerCommonMessage);
                            continue;
                        }
                        List<WorkerBank> workerBanks = workerBankDao.selectList(new QueryWrapper<WorkerBank>()
                                .lambda().eq(WorkerBank::getWorkerId, worker.getId()));
                        if (VerificationCheck.listIsNull(workerBanks)) {
                            log.error("创客未绑定银行卡" + "\t总包ID:" + panymentOrderId + "\t支付清单ID:" +
                                    paymentInventory.getId() + "\t创客姓名:" + worker.getAccountName());
                            paymentInventory.setPaymentStatus(-1);
                            paymentInventoryDao.updateById(paymentInventory);
                            CommonMessage merchantCommonMessage = getCommonMessage("ID为" + paymentInventory.getWorkerId()
                                            + "，姓名为" + paymentInventory.getWorkerName() + "的创客未绑定银行卡，只有绑定银行卡后才可以进行发薪",
                                    "", "0", UserType.ADMIN, paymentOrder.getMerchantId(), UserType.MERCHANT);
                            CommonMessage workerCommonMessage = getCommonMessage("您现在未绑定银行卡，请完成绑卡操作，才可以进行发薪操作!",
                                    "", "0", UserType.ADMIN, worker.getId(), UserType.WORKER);
                            commonMessageList.add(merchantCommonMessage);
                            commonMessageList.add(workerCommonMessage);
                            continue;
                        ...
                        paymentRequestBean.setNo_order(no_order);
                        paymentRequestBean.setDt_order(DateUtil.format(new Date(), DatePattern.PURE_DATETIME_PATTERN));
                        paymentRequestBean.setMoney_order(paymentInventory.getRealMoney()
                        ...
                        if (StringUtils.isEmpty(encryptStr)) {
                            ...
                            paymentInventoryDao.updateById(paymentInventory);
                            CommonMessage merchantCommonMessage = getCommonMessage("ID为" + paymentInventory.getWorkerId()
                                            + "，姓名" + paymentInventory.getWorkerName() + "，支付时异常，请联系服务人员！", no_order,
                                    "0", UserType.ADMIN, paymentOrder.getMerchantId(), UserType.MERCHANT);
                            CommonMessage workerCommonMessage = getCommonMessage("支付时异常，请联系服务人员!",
                                    "", "0", UserType.ADMIN, worker.getId(), UserType.WORKER);
                            commonMessageList.add(merchantCommonMessage);
                            commonMessageList.add(workerCommonMessage);
                            // 加密异常
                        ...
                        if (!("0000".equals(paymentapiResult.get("ret_code")))) {
                            Map<String, String> queryPaymentResult = this.queryPayment(lianlianpayTax.getOidPartner(),
                                    no_order, lianlianpayTax.getPrivateKey());
                            if ("0000".equals(queryPaymentResult.get("ret_code"))) {
                                if ("SUCCESS".equals(queryPaymentResult.get("result_pay").toUpperCase())
                                        || "PROCESSING".equals(queryPaymentResult.get("result_pay").toUpperCase())) {
                                    paymentInventory.setPaymentStatus(1);
                            ...
                            log.error(paymentapiResult.get("ret_msg") + "\t总包ID:" + panymentOrderId + "\t支付清单ID:" +
                                    paymentInventory.getId() + "\t创客姓名:" + worker.getAccountName());
                            paymentInventory.setPaymentStatus(-1);
                            paymentInventoryDao.updateById(paymentInventory);
                            CommonMessage merchantCommonMessage = getCommonMessage("ID为" + paymentInventory.getWorkerId() +
                ...
                senMsg("订单" + paymentOrder.getId() + "支付成功！", result.get("no_order"), "0",
                        UserType.ADMIN, paymentOrder.getMerchantId(), UserType.MERCHANT);
            }
        } catch (IOException e) {
            log.error(e + ":" + e.getMessage());
        }
    ...
    public ReturnJson merchantPayMany(String merchantId, String payPassWord, String paymentOrderId) throws CommonException {
        ...
        Lianlianpay lianlianpay = this.getOne(new QueryWrapper<Lianlianpay>()
                .lambda().eq(Lianlianpay::getCompanyId, paymentOrderMany.getCompanyId()));
        if (lianlianpay == null) {
            log.error("连连账号不存在！");
            return ReturnJson.error("请先添加连连商户号和私钥");
        }
    ...
    public void merchantManyNotifyUrl(HttpServletRequest request) {
        ...
        try {
            ...
            if (!signCheck) {
                ...
                paymentOrderMany.setPaymentOrderStatus(1);
                paymentOrderManyDao.updateById(paymentOrderMany);
                senMsg("订单" + paymentOrderMany.getId() + "数据异常，请联系服务人员！", result.get("no_order"),
                        "0", UserType.ADMIN, paymentOrderMany.getMerchantId(), UserType.MERCHANT);
                return;
            }
...
//            paymentHistoryService.saveOrUpdate(paymentHistory);
            ...
            if ("SUCCESS".equals(result.get("result_pay"))) {
                ...
                paymentOrderManyDao.updateById(paymentOrderMany);
                senMsg("订单" + paymentOrderMany.getId() + "支付成功！", result.get("no_order"),
                        "0", UserType.ADMIN, paymentOrderMany.getMerchantId(), UserType.MERCHANT);
            } else {
                //商户付款失败
                paymentOrderMany.setPaymentOrderStatus(-1);
    ...
    public Map<String, String> queryPaymentByorderId(String merchantId, String oidPaybill) throws CommonException {
        ...
        Lianlianpay lianlianpay = this.getOne(new QueryWrapper<Lianlianpay>()
                .lambda().eq(Lianlianpay::getCompanyId, merchant.getCompanyId()));
        Map<String, String> result = this.queryPayment(lianlianpay.getOidPartner(),
                "", oidPaybill, lianlianpay.getPrivateKey());
        if ("0000".equals(result.get("ret_code"))) {
            return result;
        }
        throw new CommonException(Integer.valueOf(result.get("ret_code")), result.get("ret_msg"));
