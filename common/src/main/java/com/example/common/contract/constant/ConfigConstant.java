package com.example.common.contract.constant;
...
public class ConfigConstant {
    ...
    public static final String getToken_URL(String appId, String secret) {
        return EqianbaoConfig.getHost() + "/v1/oauth2/access_token?appId=" + appId + "&secret=" + secret + "&grantType=client_credentials";
    }

    // 刷新Token
    public static final String refreshToken_URL(String appId, String refreshToken) {
        return EqianbaoConfig.getHost() + "/v1/oauth2/access_token?appId=" + appId + "&refreshToken=" + refreshToken
                + "&grantType=refresh_token";
    }

    //银行3要素认证
    public static final String bankAuthentication_URL() {
        return EqianbaoConfig.getHost() + "/v2/identity/verify/individual/bank3Factors";
    }
    ...
    public static final String createPerAcc_URL() {
        return EqianbaoConfig.getHost() + "/v1/accounts/createByThirdPartyUserId";
    }

    // 修改个人账号(根据账号ID)
    public static final String modifyPerAccById_URL(String accountId) {
        return EqianbaoConfig.getHost() + "/v1/accounts/" + accountId;
    }

    // 修改个人账号（根据第三方用户ID）
    public static final String modifyPerAccByThirdId_URL(String thirdPartyUserId) {
        return EqianbaoConfig.getHost() + "/v1/accounts/updateByThirdId?thirdPartyUserId=" + thirdPartyUserId;
    }
    ...
    public static final String queryAccByThirdId_URL(String thirdPartyUserId) {
        return EqianbaoConfig.getHost() + "/v1/accounts/getByThirdId?thirdPartyUserId=" + thirdPartyUserId;
    }
    ...
    public static final String logoutAccByThirdId_URL(String thirdPartyUserId) {
        return EqianbaoConfig.getHost() + "/v1/accounts/deleteByThirdId?thirdPartyUserId=" + thirdPartyUserId;
    }
    ...
    public static final String createOrgAcc_URL() {
        return EqianbaoConfig.getHost() + "/v1/organizations/createByThirdPartyUserId";
    }

    // 修改机构账号(根据账号ID)
    public static final String modifyOrgAccById_URL(String orgId) {
        return EqianbaoConfig.getHost() + "/v1/organizations/" + orgId;
    }

    // 修改机构账号(根据第三方用户ID)
    public static final String modifyOrgAccByThirdId_URL(String thirdPartyUserId) {
        return EqianbaoConfig.getHost() + "/v1/organizations/updateByThirdId?thirdPartyUserId=" + thirdPartyUserId;
    }
    ...
    public static final String queryOrgAccByThirdId_URL(String thirdPartyUserId) {
        return EqianbaoConfig.getHost() + "/v1/organizations/getByThirdId?thirdPartyUserId=" + thirdPartyUserId;
    }
    ...
    public static final String logoutOrgAccByThirdId_URL(String thirdPartyUserId) {
        return EqianbaoConfig.getHost() + "/v1/organizations/deleteByThirdId?thirdPartyUserId=" + thirdPartyUserId;
    }

    // ---签署授权---
    // 设置/撤销静默签署(POST为设置静默授权,DELETE为撤销静默授权)
    public static final String signAuth_URL(String accountId) {
        return EqianbaoConfig.getHost() + "/v1/signAuth/" + accountId;
    }
    ...
    public static final String fileUpload_URL() {
        return EqianbaoConfig.getHost() + "/v1/files/getUploadUrl";
    }

    // 文件模板创建待签文件
    public static final String createFileByTemplate_URL() {
        return EqianbaoConfig.getHost() + "/v1/files/createByTemplate";
    }

    // 获取文件下载地址(根据文件Id)
    public static final String fileDownloadByFileId_URL(String fileId) {
        return EqianbaoConfig.getHost() + "/v1/files/" + fileId;
    }
    ...
    public static final String createTemplateByUpload_URL() {
        return EqianbaoConfig.getHost() + "/v1/docTemplates/createByUploadUrl";
    }

    // 添加输入项组件
    public static final String addInputNodes_URL(String templateId) {

        return EqianbaoConfig.getHost() + "/v1/docTemplates/" + templateId + "/components";
    }

    // 删除输入项组件
    public static final String deleteInputNodes_URL(String templateId, String ids) {
        return EqianbaoConfig.getHost() + "/v1/docTemplates/" + templateId + "/components/" + ids;
    }

    // 查询模板详情
    public static final String queryInputNodes_URL(String templateId) {
        return EqianbaoConfig.getHost() + "/v1/docTemplates/" + templateId;
    }
    ...
    public static final String createPerSeal_URL(String accountId) {
        return EqianbaoConfig.getHost() + "/v1/accounts/" + accountId + "/seals/personaltemplate";
    }

    // 创建机构模板印章
    public static final String createOfficialSeal_URL(String orgId) {
        return EqianbaoConfig.getHost() + "/v1/organizations/" + orgId + "/seals/officialtemplate";
    }

    // 创建个人/机构图片印章
    public static final String createImageSeal_URL(String accountId) {
        return EqianbaoConfig.getHost() + "/v1/accounts/" + accountId + "/seals/image";
    }

    // 查询个人印章
    public static final String queryPerSeal_URL(String accountId, int offset, int size) {
        return EqianbaoConfig.getHost() + "/v1/accounts/" + accountId + "/seals?offset=" + offset + "&size=" + size;
    }

    // 查询机构印章
    public static final String queryOrgSeal_URL(String orgId, int offset, int size) {
        return EqianbaoConfig.getHost() + "/v1/organizations/" + orgId + "/seals?offset=" + offset + "&size=" + size;
    }
    ...
    public static final String createFlows_URL() {
        return EqianbaoConfig.getHost() + "/v1/signflows";
    }

    // 查询签署流程
    public static final String queryFlows_URL(String flowId) {
        return EqianbaoConfig.getHost() + "/v1/signflows/" + flowId;
    }

    // 开启签署流程
    public static final String startFlows_URL(String flowId) {
        return EqianbaoConfig.getHost() + "/v1/signflows/" + flowId + "/start";
    }

    // 撤销签署流程
    public static final String revokeFlows_URL(String flowId) {
        return EqianbaoConfig.getHost() + "/v1/signflows/" + flowId + "/revoke";
    }

    // 归档签署流程
    public static final String archiveFlows_URL(String flowId) {
        return EqianbaoConfig.getHost() + "/v1/signflows/" + flowId + "/archive";
    }

    // ---流程文档(请求方式决定增删或者下载)---
    public static final String aboutDocument_URL(String flowId, String fileIds) {
        String url = EqianbaoConfig.getHost() + "/v1/signflows/" + flowId + "/documents";
    ...
    public static final String aboutAttachments_URL(String flowId, String fileIds) {
        String url = EqianbaoConfig.getHost() + "/v1/signflows/" + flowId + "/attachments";
        if (StringUtils.isNotBlank(fileIds)) {
            url += "?fileIds=" + fileIds;
        }
        return url;
    ...
    public static final String getSignFields_URL(String flowId, String accountId, String signfieldIds) {
        String url = EqianbaoConfig.getHost() + "/v1/signflows/" + flowId + "/signfields";
        boolean flag = false;
    ...
    public static final String addAutoSignfieldsForPlatform_URL(String flowId) {
        return EqianbaoConfig.getHost() + "/v1/signflows/" + flowId + "/signfields/platformSign";
    }

    // 添加流程签名域(用户自动盖章签名域)
    public static final String addAutoSignfieldsForPerson_URL(String flowId) {
        return EqianbaoConfig.getHost() + "/v1/signflows/" + flowId + "/signfields/autoSign";
    }

    // 添加流程签名域(用户手动盖章签名域)
    public static final String addHandSignfieldsForPerson_URL(String flowId) {
        return EqianbaoConfig.getHost() + "/v1/signflows/" + flowId + "/signfields/handSign";
    }

    // 删除流程签名域
    public static final String deleteSignfields_URL(String flowId, String signfieldIds) {
        return EqianbaoConfig.getHost() + "/v1/signflows/" + flowId + "/signfields?signfieldIds=" + signfieldIds;
    }
    ...
    public static final String getSignersList_URL(String flowId) {
        return EqianbaoConfig.getHost() + "/v1/signflows/" + flowId + "/signers";
    }

    // 流程签署人催签
    public static final String urgeSgin_URL(String flowId) {
        return EqianbaoConfig.getHost() + "/v1/signflows/" + flowId + "/signers/rushsign";
    }

    // 获取签署地址
    public static final String Sign_URL(String flowId, String accountId, String organizeId, String urlType) {
        String url = EqianbaoConfig.getHost() + "/v1/signflows/" + flowId + "/executeUrl?accountId=" + accountId;
    ...
    public static final String getKeywordsPosition_URL(String flowId, String fileId) {
        ...
        keywordsPosition.append(EqianbaoConfig.getHost()).append("/v1/signflows/");
